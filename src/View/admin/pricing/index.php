    ...
    <form method="POST" action="/admin/tarifs">
        ...
        <div class="admin-table-wrap">
            <table class="admin-table">
                ...
                <tbody>
                <?php if (empty($rules)) : ?>
                    <tr><td colspan="8" style="text-align:center;color:#8a7a60;padding:2rem;">Aucune règle de tarification</td></tr>
                <?php else : ?>
                    <?php foreach ($rules as $rule) : ?>
                        ...
                        <tr>
                            ...
                            <td><?= $rule['max_quantity'] !== null ? (int) $rule['max_quantity'] : '∞' ?></td>
                            <td>
                                <input type="number" name="delivery_<?= (int) $rule['id'] ?>"
                                       min="0" step="0.01"
                                       value="<?= htmlspecialchars(number_format((float) $rule['delivery_price'], 2, '.', '')) ?>">
                            </td>
                            <td>
                                <input type="number" name="withdrawal_<?= (int) $rule['id'] ?>"
                                       min="0" step="0.01"
                                       value="<?= htmlspecialchars(number_format((float) $rule['withdrawal_price'], 2, '.', '')) ?>">
                            </td>
                            <td>
                                <input type="text" name="label_fr_<?= (int) $rule['id'] ?>"
                                       value="<?= htmlspecialchars(pricingLabel($rule['label'], 'fr')) ?>">
                            </td>
                            <td>
                                <input type="text" name="label_en_<?= (int) $rule['id'] ?>"
                                       value="<?= htmlspecialchars(pricingLabel($rule['label'], 'en')) ?>">
                            </td>
                            <td style="text-align:center;">
                                <input type="hidden" name="active_<?= (int) $rule['id'] ?>" value="0">
                                <input type="checkbox" name="active_<?= (int) $rule['id'] ?>" value="1"
                                       <?= $rule['active'] ? 'checked' : '' ?>
        ...
        <?php if (!empty($rules)) : ?>
        <div style="padding:1.25rem 1.5rem;border-top:1px solid rgba(8,8,8,0.07);display:flex;justify-content:flex-end;">
