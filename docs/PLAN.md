...
| `Order-Management.php` | `src/View/admin/orders/` — liste (per_page + filtre statut/paiement) + détail (upload facture PDF, téléchargement sécurisé admin + client propriétaire, masqué si commande annulée) | 🔄 Implémenté |
...
| `feat/admin`             | 🔄 En cours  | Back-office : dashboard, vins CRUD, commandes, comptes, tarifs, actualités, newsletter — UX round 8 terminé (commandes : per_page, filtre paiement, upload facture PDF sécurisé + fix bug $status/EXTR_SKIP dans Response::view) |
