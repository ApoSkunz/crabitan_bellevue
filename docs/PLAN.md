...
| `Bad-Request.php` | `src/View/errors/error.php` (400) | ✅ Validé |
| `Unauthorized.php` | `src/View/errors/error.php` (401) | ✅ Validé |
| `Access-Forbidden.php` | `src/View/errors/error.php` (403) | ✅ Validé |
| `Not-Found.php` | `src/View/errors/error.php` (404) | ✅ Validé |
| `Method-Not-Allowed.php` | `src/View/errors/error.php` (405) | ✅ Validé |
...
**30 validées — 2 implémentées — 10 à créer**
...
| `feat/account`           | 🔄 En cours  | Espace client : commandes, adresses, favoris, sécurité, export RGPD, suppression compte — branch `feat/account-views` |
| `feat/account-reactivate` | ⬜ À faire   | Réactivation de compte dans le délai de 30 jours : AuthController détecte soft-delete → page intermédiaire + lien de réactivation dans email de suppression |
| `feat/rgpd-inactive-accounts` | ⬜ À faire | Suppression automatique des comptes inactifs (configurable, défaut 36 mois) : email J-30, soft-delete, cron, protection admins/commandes actives |
...
| `chore/maintenance-page` | ✅ Fait       | Page de maintenance standalone (`public/maintenance.php`) : light theme par défaut, toggle jour/nuit identique au site (&#9728;/&#9790;, même clé localStorage `cb-theme`), logo 160px, HTTP 503 + Retry-After |
| `feat/order-address-snapshot` | ⬜ À faire | Snapshot des adresses sur les commandes : stocker les champs adresse directement dans `orders` (billing_firstname, billing_street… delivery_firstname…) au moment du checkout, pour découpler définitivement les adresses actives du récapitulatif de commande et supprimer la contrainte FK qui bloque la suppression/modification d'adresses |

---

## Backlog détaillé

### feat/rgpd-inactive-accounts

> Conformité RGPD — principe de minimisation des données (art. 5 §1 e) : les données personnelles ne doivent pas être conservées au-delà de la durée nécessaire à leur finalité. Un compte inactif depuis une longue période sans commande en cours doit être supprimé (soft-delete + anonymisation différée comme le mécanisme existant).

**Critère d'inactivité :** aucune connexion (`connections.created_at`) depuis **X mois** (valeur configurable via `RGPD_INACTIVITY_MONTHS` dans `.env`, défaut recommandé : 36 mois) ET aucune commande active (statut `pending` / `processing`).

**Périmètre :**

- [ ] **Commande SQL de détection** : `SELECT a.id FROM accounts a LEFT JOIN connections c ON c.user_id = a.id WHERE a.deleted_at IS NULL AND (c.id IS NULL OR MAX(c.created_at) < DATE_SUB(NOW(), INTERVAL ? MONTH)) AND a.id NOT IN (SELECT DISTINCT user_id FROM orders WHERE status IN ('pending','processing')) GROUP BY a.id`
- [ ] **`AccountModel::getInactiveAccounts(int $months): array`** — retourne les IDs éligibles
- [ ] **Email d'avertissement** — envoyer un mail 30 jours avant la suppression : "Votre compte sera supprimé le JJ/MM/AAAA faute d'activité. Connectez-vous pour le conserver."
  - Nouveau template `MailService::sendInactivityWarning()`
  - Stocker `inactivity_warning_sent_at` sur `accounts` pour ne pas renvoyer à chaque run
- [ ] **Migration** : ajouter colonne `inactivity_warning_sent_at DATETIME NULL` sur `accounts`
- [ ] **Commande / job** : `php bin/console rgpd:purge-inactive` (ou appel CRON) — deux passes :
  1. Envoi des avertissements (J-30)
  2. Soft-delete (`AccountModel::delete()`) + révocation des sessions actives pour les comptes dont le délai est écoulé
- [ ] **Cron GitHub Actions** (ou script XAMPP cron) — planification quotidienne
- [ ] **Protection** : ne jamais supprimer les comptes `admin` / `super_admin`, ni ceux ayant une commande active
- [ ] **Traçabilité admin** : nouvelle colonne `Inactif depuis` dans la vue `/admin/comptes` (date de dernière connexion)
- [ ] **Tests** : TU `AccountModelInactiveTest`, TI avec scénario 3 ans sans connexion

**Acceptance criteria :**
- Un compte sans connexion depuis 36 mois et sans commande active reçoit l'email J-30 puis est soft-deleted
- Une connexion (même un jour avant l'échéance) réinitialise le compteur → compte conservé
- Les admins et comptes avec commandes actives sont protégés
- Le process est idempotent (re-exécution sans double suppression ni double mail)

---

### feat/order-address-snapshot

> Actuellement les commandes référencent `id_billing_address` et `id_delivery_address` via FK. Cela bloque la suppression/modification d'adresses même après livraison, et expose au risque de divergence si une adresse est modifiée. La solution propre est de stocker un snapshot des données d'adresse directement dans la table `orders` au moment du passage de commande.

**Périmètre :**
- Ajouter ~12 colonnes dans `orders` : `billing_firstname`, `billing_lastname`, `billing_street`, `billing_zip_code`, `billing_city`, `billing_country`, `billing_phone`, et les 6 équivalents delivery
- Au checkout : copier les champs de l'adresse sélectionnée dans ces colonnes (snapshot immuable)
- Conserver les FK `id_billing_address` / `id_delivery_address` en NULLABLE pour la traçabilité, mais ne plus dépendre d'elles pour l'affichage des commandes
- Supprimer les contraintes FK bloquantes → la suppression/modification d'adresses devient libre
- Mettre à jour les vues commande (admin + espace client) pour lire le snapshot, pas l'adresse liée
- Supprimer la logique de soft-lock des adresses (`getAddressIdsWithActiveOrders`) une fois le snapshot en place

**Bénéfice :** gestion des adresses simplifiée, données de commande pérennes indépendantes des évolutions du carnet d'adresses.
